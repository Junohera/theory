...
# 🎁 Command TIP
# when file✅
scp ${TARGET_FILE} ${USERNAME}@${HOST}:${PATH}
# when directory
scp -r ${TARGET_DIRECTORY} ${USERNAME}@${HOST}:${PATH}
```
...
- [preinstall](https://oracle-base.com/articles/12c/oracle-db-12cr2-installation-on-oracle-linux-6-and-7)
  ...
  - `ps -ef | grep yum | grep -v grep`  실행 후, 아무것도 없을 경우 yum install 실행

    ```shell
    # 🎁 open another session
    vi is_complete_yum_process.sh
    
    #!/bin/sh
    
    clear
    
    operation_time_start=$(echo $(date +%S))
    while [ 1 ]
    do
      ps -ef | grep yum | grep -v grep | grep -v $0 > /dev/null
      if [ $? -eq 1 ];
        then
        	operation_time_end=$(echo $(date +%S))
        	operation_time=$(($operation_time_end - $operation_time_start))
          echo "END YUM PROCESS $(date +%F_%T), OPERATION_TIME: $operation_time"
          exit
        else
          echo "NOT YET $(date +%F_%T)"
      fi
      sleep 1
    done
    
    sh is_complete_yum_process.sh
    ```
...
- as a oracle(X Window System protocol을 xLaunch로서 시작할 경우, putty를 재접속해야 연동)
...
- (Optional) set alias as a oracle
  ...
  alias 'sss=c;cd /oracle12/admin/;sqlplus / as sysdba'
