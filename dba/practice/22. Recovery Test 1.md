...
7. 복구방식 판단✅
   archive모드이면서 redo와 archive log file이 정상으로 판단된다면 우선 **완전복구**를 진행해본다.
   archive모드가 아니거나 redo 또는 archive log file이 비정상으로 판단된다면 어쩔 수 없이 **불완전복구**를 진행해야한다.
...
특정 시퀀스에 대응하는 archive log file이 없을 경우,
정황상 archive에 떨어지지 못하고, current상태의 redo log에 남아있을 수도 있음.
...
instance recovery를 기대하여 위와같이 하지만 resetlogs만 가능
